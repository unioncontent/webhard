...
var Keyword = {
  table: function(){
    return global.osp.replace('_admin','') + '_cnts_kwd_f';
  },
  updateKeyword: function(item,callback){
    var sql = 'update '+this.table()+' set K_method=?, K_key=?, K_apply=? where n_idx=?';
    var param = [item.K_method,item.K_key,item.K_apply,item.n_idx_k];
    connection.query(sql,param,callback);
  },
  updateAllKeyword: function(item,callback){
    var sql = 'update '+this.table()+' set K_method=?, K_key=?, K_apply=? where n_idx_c=?';
  ...
  insertKeyword: function(item,callback) {
    ...
    var sql = 'insert '+this.table()+'(n_idx_c, U_id_c, K_keyword, K_apply, K_method, K_key, K_type, K_regdate) values(?,?,?,?,?,?,?,now())';
    console.log(sql,param);
    connection.query(sql,param,callback);
  },
  getKeyInfo: function(idx,callback){
    var sql = 'select * from '+this.table()+' where n_idx_c=?';
