...
$('#imgUpload').on('click', function () {
  ...
  $.ajax({
      type: 'post',
      url: 'http://61.82.113.197:8090/setting/file_upload',
      data: formData,
      processData: false,
      contentType: false,
      success: function (data) {
        ...
        $('#cp_logo').val(data);
        // $('.img-cLogo').attr("src","http://61.82.113.197:8090/"+data);
      },
      error: function (err) {
          console.log(err);
...
function ajaxFileDel(filePath,id,type){
  ...
  $.ajax({
    type: 'post',
    url: 'http://61.82.113.197:8090/setting/file_delete',
    ...
    if (value != null) {
      ...
      $.ajax({
        ...
        success: function(data){
          if(data.state){
            alert('수정 완료했습니다.');
            console.log($('#cp_logo').val());
            console.log($('#cp_logo_txt').val());
