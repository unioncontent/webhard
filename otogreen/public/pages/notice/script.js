...
$('#file_input').on('change',function(){
  ...
  $.ajax({
      type: 'post',
      url: 'http://61.82.113.197:8090/notice/file_upload',
      data: formData,
      processData: false,
      contentType: false,
...
function ajaxFileDel(filePath,tag){
  $.ajax({
    type: 'post',
    url: 'http://61.82.113.197:8090/notice/file_delete',
    data: {path:filePath},
    success: function (data) {
      console.log(data);
      $(tag).remove();
      var param = { idx : $('.btn-delete').data('idx') };
      ...
      $.ajax({
        ...
        success: function(data){
          if(data.state){
            console.log('파일수정 성공했습니다.');
          }
          else{
            console.log('파일수정 실패했습니다.');
          }
        }
      });
    },
...
function insertNotice(param){
  ...
  if($('#uploadFileList li').length > 0){
    var arr = [];
    $.each( $('#uploadFileList li') , function( key, value ) {
      arr.push($(value).find('.file-del').data('path').replace('public/notice',''));
    });
    param.file = arr.join(',');
  }
  $.ajax({
