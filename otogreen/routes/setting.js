...
var mailing = require('../models/mailing.js');
...
router.post('/:pType/update',isAuthenticated,async function(req, res, next) {
  ...
  if(req.params.pType == 'osp'){
    ...
    var type = (req.body.cp_class == 'c')?'k_cp':'k_mcp';
    data = await cp.update_k(type,[req.body.cp_state,idx]);
  }
  else{
...
router.post('/:pType/getInfo',isAuthenticated,async function(req, res, next) {
  ...
  if(req.params.pType == 'osp'){
    ...
    if(data.osp_id != ''  && data.osp_id != null){
      data.graph = await osp.getSixOSPData(data.osp_id);
    }
  }
  else if(req.params.pType == 'cp'){
