...
var cp = {
  ...
  selectViewCount: async function(body,param){
    var sql = 'select count(*) as total from cp_list where n_idx is not null ';
    if('class' in body){
      sql += ' and cp_class = \''+body['class']+'\'';
    }
    if('searchType' in body){
  ...
  delete: async function(param) {
    ...
    try {
      await funDB.getResult('a',sql,param['val']);
    } catch (e) {
      console.log(e)
    } finally {
      return await funDB.getResult('b',sql,param['val']);
    }
  },
  ...
  update_k: async function(type,param) {
    var sql = "update k_word set k_state =? where "+type+" in (select cp_id from cp_list where n_idx=?)";
    try {
